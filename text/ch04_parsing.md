...
Շատ հետաքրքիր ու կարևոր է հրամանների հաջորդականության՝ `Sequence`, վերլուծությունը։ Քերականությունից տեսնում ենք, որ `Sequence`-ով ան արտահայտված ենթածրագրի մարմինը, `IF` կառուցվածքի դրական ու բացասական ճյուղերի մերմինները, `WHIEL` և `FOR` կրկնման կառուցվածքների մարմինները։
...
$$
FIRST(Statement) \equiv \{ DIM, LET, INPUT, PRINT, IF, WHILE, FOR, CALL \}
$$
