...
Բալ _ծրագիրը_՝ `Program`, ենթածրագրերի հաջորդականություն է։ Դա արտահայտվում է հետևյալ _քերականական հավասարմամբ_.

```
Program = { Subroutine }.
```

Այստեղ `{` և `}` փակագծերը նշանակում են «պարփակված արտահայտության զրո կամ ավելի կրկնություններ»։ Տվյալ դեպքում՝ `Subroutine` տարրի զրո և ավելի անգամ կրկնություններ։ 
...
Այս հավասարման մեջ օգտագործված `[` և `]` փակագծերով որոշվում է «պարփակված արտահայտության առկայություն կամ բացակայություն»։ Այսինքն՝ տվյալ դեպքում նշված է, որ ենթածրագրի պարամետրերը պարտադիր չեն։
